...
function fcnProcessMatchTCG() {
  ...
  Logger.log("Routine: fcnProcessMatchTCG");
  // Opens Spreadsheet
    ...
    if (DataCopiedStatus == 'Data Copied'){
      ...
      if (EntriesProcessing == 1){
        ...
        while (EntriesProcessing >= 1) {
          Status = fcnAnalyzeResultsTCG(ss, shtConfig, cfgEvntParam, cfgColRspSht, cfgColRndSht, cfgColMatchRep, cfgExecData, shtRspn);
          EntriesProcessing = shtRspn.getRange(1, colNbUnprcsdEntries).getValue();
...
function fcnAnalyzeResultsTCG(ss, shtConfig, cfgEvntParam, cfgColRspSht, cfgColRndSht, cfgColMatchRep, cfgExecData, shtRspn) {
  ...
  var exePostMatchEvntLog =    cfgExecData[ 9][0];
  var exePostProcessLog =      cfgExecData[10][0];
  ...
  var evntNbCardsTCG =     cfgEvntParam[42][0];
  ...
  // Column: 0 = Card in Pack, 1 = Card Number,   2 = Card Name, 3= Card Rarity
  var PackData = subCreateArray(evntNbCardsTCG,4);
  ...
  for (var RspnRow = RspnNextRowPrcss; RspnRow <= RspnMaxRows; RspnRow++){
    ...
    if (RspnDataRoundNum != '' && RspnDataPrcssd == ''){
      ...
      // Post the Match Results in the Event Log if Enabled
      if(exePostMatchEvntLog == 'Enabled'){
      ...
      if(Status[0] < 0) {
        ...
        Logger.log('Process Error Emails Sent');
      }
