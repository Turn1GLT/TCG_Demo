...
function fcnProcessMatchTCG() {
    ...
    if (DataCopied == 1){
      ...
      if (EntriesProcessing == 1){
        ...
        while (EntriesProcessing >= 1) {
          Status = fcnAnalyzeResultsTCG(ss, shtConfig, ConfigData, shtRspn);
          EntriesProcessing = shtRspn.getRange(1, ColNbUnprcsdEntries).getValue();
  ...
  if (cfgSendLog == 'Enabled' || (EmailValid == 0 && Email != '')){
    ...
    var subject = 'TCG Match Process Log - ' + shtConfig.getRange(11,2).getValue() + ' ' + shtConfig.getRange(13,2).getValue() + '- Entry Row ' + RspnRow;
...
function fcnAnalyzeResultsTCG(ss, shtConfig, ConfigData, shtRspn) {
            ...
            if (OptPostResult == 'Enabled'){
              ...
              if (MatchPostStatus == 1){
                ...
                if (OptTCGBooster == 'Enabled'){
                  for (var card = 0; card < cfgNbCards; card++){
